...
class Network
{
  ...
  public:
    ...
    void get_bondDEnergy( int bi, const std::vector<double> &r,  std::vector<double> &F) const
        {  bonds[bi].dEnergy(r, F, this); }

    double get_bondEdE( int bi, const std::vector<double> &r, std::vector<double> &F) const
...
double Network::Bond::get_l( const std::vector<double> &r, const Network *net) const
{
...
void Network::Bond::dEnergy( const std::vector<double> &r, std::vector<double> &F, const Network *net) const
{
    double g = (net->gamma)*(net->Ly);
	double dx = r[2*i]   - r[2*j]   - (net->Lx)*(1+net->epsilonX)*xb - yb*g;
	double dy = r[2*i+1] - r[2*j+1] - (net->Ly)*(1+net->epsilonY)*yb;
...
double Network::Bond::energy_dEnergy(const std::vector<double> &r, std::vector<double> &F, const Network *net) const
{
